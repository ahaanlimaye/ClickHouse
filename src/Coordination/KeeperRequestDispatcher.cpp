...
void KeeperRequestDispatcher::requestThread()
{
...
bool KeeperRequestDispatcher::setResponse(int64_t session_id, const Coordination::ZooKeeperResponsePtr & response, Coordination::ZooKeeperRequestPtr request)
{
...
bool KeeperRequestDispatcher::putLocalReadRequest(const Coordination::ZooKeeperRequestPtr & request, int64_t session_id)
{
    {
        ProfiledMutexLock lock(live_sessions_mutex, ProfileEvents::KeeperLiveSessionsLockWaitMicroseconds, ProfileEvents::KeeperLiveSessionsLockHoldMicroseconds);
...
void KeeperRequestDispatcher::shutdown()
{
    if (requests_queue)
...
void KeeperRequestDispatcher::addErrorResponses(const KeeperRequestsForSessions & requests_for_sessions, Coordination::Error error)
{
...
nuraft::ptr<nuraft::buffer> KeeperRequestDispatcher::forceWaitAndProcessResult(
